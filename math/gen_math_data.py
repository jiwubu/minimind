...
- 数字一律紧凑十进制，模板不再区分带空格/紧凑两套（tokenizer 已保证
  位值对齐），只保留问法多样性；
- 新增 sample_leadzero_pair：结果需去掉 k 个前导零，k 均匀覆盖（§7.2）；
- sample_negative_pair 的等长档改为"共享高位前缀、末 k 位分出大小"，
  覆盖 v5 负数档全部错例的形状（§7.1）；
...
def sample_repdigit_pair(max_digits):
    ...
    竖式 '9 9 + 9 9 9 9'。v6 patched tokenizer 下这道伤不存在了，但
    这一档**仍然保留**：§6.3 已证明长重复串数不清位数的公共根因是
...
def sample_leadzero_pair(max_digits):
    """采样结果需去掉 k 个前导零的减法对（v6 新增，方案 2 §7.2）。
...
def sample_negative_pair(max_digits):
    ...
    2. 等长且接近（v6 主修，§7.1）：v5 负数档的全部错例（14-15、
    ...
    ≥2 位"的严格口径下实测 ≈2.1%（小位数样本稀释），达到 §7.1 的
...
def make_sample(max_digits, neg_ratio, close_ratio=0.12, rep_ratio=0.08,
    ...
    if r < close_ratio:
        ...
        # （sample_leadzero_pair，补 §6.2 的中间深度空档）
        if random.random() < 0.5:
